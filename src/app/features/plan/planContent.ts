...
export const PLAN_DAYS: PlanDay[] = [
  /* ---------------- WEEK 1 — Know what is actually happening ------------- */
  {
    ...
    why: 'This is the measuring stick for the whole six weeks. The goal is one listing from someone NOT on this list — and without the list written down, you will not be able to prove it happened. Do it first because everything after it is measured against it.',
    minutes: 45,
  ...
  },
  {
    ...
    theme: 'Know what is actually happening',
    category: 'Search',
    action: 'Write the words for a "Casas en venta en Piura" page: what someone searching that phrase should read in the first ten seconds. Half a page, in Spanish.',
    why: 'This is the legitimate version of local discovery for a digital business — a page per city that Google can rank. It is also the growth path: the same page shape becomes Trujillo, Chiclayo, Lima. I will build the page; the local knowledge has to come from you.',
    minutes: 45,
    doneWhen: 'Half a page of Spanish copy written and sent to me.',
    spend: 0, owner: 'jerson',
  },
  {
  ...
  },
  {
    ...
    action: 'Find the 6 largest Piura property groups on Facebook, request to join, AND read each one\'s rules on commercial or agent posts. Write down name, member count, and whether listings are allowed.',
    why: 'Eight later days assume these groups exist, are active, and permit this kind of post. That assumption is unverified — many groups ban commercial posts outright. This day exists to test it before anything depends on it. If most of them forbid listings, tell me on day 5 and we replace that third of the plan with Marketplace and search rather than discovering it on day 9.',
    minutes: 35,
    doneWhen: '6 groups found, requests sent, and each one marked as allows-listings or does-not.',
    spend: 0, owner: 'jerson',
  },
  {
  ...
  },
  {
    ...
    theme: 'Know what is actually happening',
    category: 'Review',
    action: 'Week review. Check: is Search Console showing anything yet? Were the Facebook group requests accepted? Is the recruited list complete?',
  ...
  /* ---------------- WEEK 2 — Be findable -------------------------------- */
  {
    ...
    theme: 'Be findable',
    category: 'Search',
    action: 'Pick the 6 Castilla listings that should appear on the Castilla page, and write one honest line about the district itself — who actually wants to live there.',
    why: 'A district page that reads like it was written by someone who knows Castilla will outrank a generic portal page. This is the one advantage a local team has over Urbania, and it costs nothing.',
    minutes: 40,
    doneWhen: '6 listings chosen and the district line written.',
    spend: 0, owner: 'jerson',
  },
  {
  ...
  },
  {
    ...
    theme: 'Prepare paid',
    category: 'Paid setup',
    action: 'Declare the campaign as Housing, then build the audience with LOCATION ONLY: a radius around Piura, minimum 15km. Do not set age, gender or interests — Meta will not allow them.',
    why: 'Meta classifies property ads as housing, which strips out age, gender and interest targeting and forces a 15km minimum radius. Since 2026 it detects property photos automatically and applies this even if you do not declare it — so declaring it yourself avoids a rejection. The practical consequence is the next day matters more: with targeting reduced to geography, the advert itself has to do the filtering.',
    minutes: 40,
    doneWhen: 'A housing campaign exists with a Piura radius of 15km or more, and no age, gender or interest filters set.',
    spend: 0, owner: 'jerson',
  },
  {
    ...
    theme: 'Prepare paid',
    category: 'Paid setup',
    action: 'Write 3 different adverts for the same Castilla property — one price-led, one photo-led, one question-led. Name the district and the price in the first line of each.',
    why: 'Because housing rules strip the targeting down to geography, the advert has to select its own audience. Naming "Castilla" and the price in the first line means the wrong people scroll past and the right people stop — that filtering now has to happen in the words, not the settings.',
    minutes: 45,
  ...
  },
  {
    ...
    theme: 'Prepare paid',
    category: 'Search',
    action: 'Open Search Console and write down the 5 phrases people actually typed before Oqupa appeared. Compare them to what you assumed.',
    why: 'Three weeks in, this is the first real evidence of what Piura searches for. It usually contradicts the assumption, and it should steer the next round of pages.',
    minutes: 25,
    doneWhen: 'The 5 phrases written down, with a note on which were surprising.',
    spend: 0, owner: 'jerson',
  },
  {
